/**
 * Script to list all premium users in Firestore
  ...
  try {
    ...
    usersSnapshot.docs.forEach(doc => {
      ...
      const userInfo = {
        ...
        subscriptionStartDate: userData.subscription?.subscriptionStartDate ?
                              new Date(userData.subscription.subscriptionStartDate.seconds * 1000).toLocaleDateString() :
                              'N/A',
        subscriptionEndDate: userData.subscription?.subscriptionEndDate ?
                            new Date(userData.subscription.subscriptionEndDate.seconds * 1000).toLocaleDateString() :
                            'N/A',
    ...
    if (freeUsers.length > 0) {
      ...
      freeUsers.slice(0, 10).forEach((user, index) => {
        console.log(`${index + 1}. ${user.displayName} (${user.email}) - UID: ${user.uid}`);
      });
