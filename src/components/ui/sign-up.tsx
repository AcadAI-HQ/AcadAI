...
const GlassButton = React.forwardRef<HTMLButtonElement, GlassButtonProps>(({ className, children, size, contentClassName, onClick, ...props }, ref) => {
  const handleWrapperClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Only forward click if the click originated OUTSIDE the button itself;
    // clicks inside the button already bubble up and trigger onClick naturally.
...
// --- GRADIENT BACKGROUND (landing-page palette) ---
export const GradientBackground = () => (
  <>
    <style>{`
      @keyframes auth-float1 {
        0%   { transform: translate(0px,    0px)   scale(1);    opacity: 0.90; }
        33%  { transform: translate(-22px,  18px)  scale(1.06); opacity: 1;    }
        66%  { transform: translate(14px,   26px)  scale(0.96); opacity: 0.88; }
        100% { transform: translate(0px,    0px)   scale(1);    opacity: 0.90; }
      }
      @keyframes auth-float2 {
        0%   { transform: translate(0px,   0px)    scale(1);    opacity: 0.75; }
        40%  { transform: translate(20px, -22px)   scale(1.07); opacity: 0.88; }
        75%  { transform: translate(-12px, -14px)  scale(0.97); opacity: 0.78; }
      ...
      @keyframes auth-float3 {
        ...
        50%  { transform: translate(16px, 18px) scale(1.10); opacity: 0.70; }
    ...
    <svg width="100%" height="100%" viewBox="0 0 800 600" fill="none" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice" className="absolute top-0 left-0 w-full h-full">
      <defs>
        {/* Primary brand blue — bottom-left */}
        <radialGradient id="ag_blue_main" cx="50%" cy="50%" r="50%">
          <stop offset="0%"   stopColor="#3B82F6" stopOpacity="0.38" />
          <stop offset="100%" stopColor="#3B82F6" stopOpacity="0"    />
        </radialGradient>
        {/* Cyan-blue — top-right */}
        <radialGradient id="ag_cyan" cx="50%" cy="50%" r="50%">
          <stop offset="0%"   stopColor="#29ABE2" stopOpacity="0.34" />
          <stop offset="100%" stopColor="#29ABE2" stopOpacity="0"    />
        </radialGradient>
        {/* Deeper blue — bottom-right */}
        <radialGradient id="ag_blue_deep" cx="50%" cy="50%" r="50%">
          <stop offset="0%"   stopColor="#2563EB" stopOpacity="0.22" />
          <stop offset="100%" stopColor="#2563EB" stopOpacity="0"    />
        </radialGradient>
        {/* Subtle purple accent — top-left */}
        <radialGradient id="ag_purple" cx="50%" cy="50%" r="50%">
          <stop offset="0%"   stopColor="#8E2DE2" stopOpacity="0.16" />
          <stop offset="100%" stopColor="#8E2DE2" stopOpacity="0"    />
        </radialGradient>

        <filter id="ag_blur_xl" x="-70%" y="-70%" width="240%" height="240%"><feGaussianBlur stdDeviation="58"/></filter>
        <filter id="ag_blur_lg" x="-60%" y="-60%" width="220%" height="220%"><feGaussianBlur stdDeviation="44"/></filter>
        <filter id="ag_blur_md" x="-50%" y="-50%" width="200%" height="200%"><feGaussianBlur stdDeviation="32"/></filter>
      </defs>

      {/* Group 1 — brand blue bottom-left + cyan top-right */}
      <g style={{ animation: 'auth-float1 26s ease-in-out infinite' }}>
        <ellipse cx="140" cy="510" rx="330" ry="250" fill="url(#ag_blue_main)" filter="url(#ag_blur_xl)" transform="rotate(-18 140 510)" />
        <ellipse cx="650" cy="100" rx="290" ry="230" fill="url(#ag_cyan)"      filter="url(#ag_blur_lg)" transform="rotate(14 650 100)"  />
      </g>

      {/* Group 2 — deeper blue bottom-right + purple top-left */}
      <g style={{ animation: 'auth-float2 34s ease-in-out infinite' }}>
        <circle  cx="710" cy="490" r="210"           fill="url(#ag_blue_deep)" filter="url(#ag_blur_xl)" />
        <ellipse cx="65"  cy="110" rx="230" ry="165" fill="url(#ag_purple)"   filter="url(#ag_blur_md)" style={{ animation: 'auth-float3 22s ease-in-out infinite' }} />
      </g>
    </svg>
  </>
);
...
function getFirebaseErrorMessage(error: any): string {
  ...
  if (code === 'auth/email-not-verified' || error?.message?.includes('verify your email'))
...
const DefaultLogo = () => (
  <div className="bg-primary text-primary-foreground rounded-md p-1.5">
    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 2a5 5 0 0 1 5 5v3a5 5 0 0 1-10 0V7a5 5 0 0 1 5-5z"/><path d="M2 20a10 10 0 0 1 20 0"/></svg>
  </div>
);

export const AuthComponent = ({ logo = <DefaultLogo />, brandName = "AcadAI", mode = 'signup' }: AuthComponentProps) => {
  ...
  // Different loading steps per mode
  const modalSteps = mode === 'login'
    ? [
        { message: "Logging you in…",  icon: <Loader className="w-12 h-12 text-[#3B82F6] animate-spin" /> },
        { message: "Welcome back!",    icon: <PartyPopper className="w-12 h-12 text-green-500" /> },
      ]
    : [
        { message: "Creating account…", icon: <Loader className="w-12 h-12 text-[#3B82F6] animate-spin" /> },
        { message: "Setting things up…",icon: <Loader className="w-12 h-12 text-[#3B82F6] animate-spin" /> },
  ...
  const handleLoginSubmit = useCallback(async () => {
    ...
    try {
      await login(email, password); // auth context redirects on success
    } catch (error: any) {
      const msg = getFirebaseErrorMessage(error);
      if (msg) { setModalErrorMessage(msg); setModalStatus('error'); }
      else      { closeModal(); }
    }
  }, [email, password, isPasswordValid, login]);
  ...
  const handleSignupSubmit = useCallback(async () => {
    ...
    setModalStatus('loading');
    try {
      await signup(email, password); // auth context redirects on success
      fireSideCanons();
    } catch (error: any) {
      const msg = getFirebaseErrorMessage(error);
  ...
  const handleFinalSubmit = (e: React.FormEvent) => {
    ...
    if (mode === 'login') {
      if (authStep === 'password') handleLoginSubmit();
    } else {
      if (authStep === 'confirmPassword') handleSignupSubmit();
    }
  };
  ...
  const handleProgressStep = () => {
    if (authStep === 'email') {
      ...
      if (mode === 'login') {
        if (isPasswordValid) handleLoginSubmit();
      } else {
  ...
  const headings = {
    email: mode === 'login' ? 'Welcome back' : 'Get started today',
    password: mode === 'login' ? 'Enter your password' : 'Create a password',
    confirmPassword: 'One last step',
  };
    ...
    <AnimatePresence>
      {modalStatus !== 'closed' && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-white/90 backdrop-blur border border-black/[0.08] rounded-2xl p-8 w-full max-w-sm flex flex-col items-center gap-4 mx-4 shadow-xl">
            {(modalStatus === 'error' || modalStatus === 'success') && (
              <button onClick={closeModal} className="absolute top-3 right-3 p-1 text-black/40 hover:text-black/70 transition-colors">
                <X className="w-5 h-5" />
              </button>
            )}
            {modalStatus === 'error' && (
              <>
                ...
                <p className="text-base font-medium text-center text-gray-800">{modalErrorMessage}</p>
              ...
              <TextLoop interval={TEXT_LOOP_INTERVAL} stopOnEnd={true}>
                {modalSteps.slice(0, -1).map((step, i) => (
                  <div key={i} className="flex flex-col items-center gap-4">
                    {step.icon}
                    <p className="text-base font-medium text-gray-800">{step.message}</p>
                  </div>
                ))}
              </TextLoop>
            )}
            {modalStatus === 'success' && (
              <div className="flex flex-col items-center gap-4">
                ...
                <p className="text-base font-medium text-gray-800">{modalSteps[modalSteps.length - 1].message}</p>
              </div>
            )}
          </motion.div>
        </motion.div>
  ...
  return (
    <div className="bg-background min-h-screen w-screen flex flex-col">
      <style>{`
        ...
        .glass-button-wrap{--anim-time:400ms;--anim-ease:cubic-bezier(0.25,1,0.5,1);--border-width:clamp(1px,0.0625em,4px);position:relative;z-index:2;transform-style:preserve-3d;transition:transform var(--anim-time) var(--anim-ease)}.glass-button-wrap:has(.glass-button:active){transform:rotateX(25deg)}.glass-button-shadow{--shadow-cutoff-fix:2em;position:absolute;width:calc(100% + var(--shadow-cutoff-fix));height:calc(100% + var(--shadow-cutoff-fix));top:calc(0% - var(--shadow-cutoff-fix)/2);left:calc(0% - var(--shadow-cutoff-fix)/2);filter:blur(clamp(2px,0.125em,12px));transition:filter var(--anim-time) var(--anim-ease);pointer-events:none;z-index:0}.glass-button-shadow::after{content:"";position:absolute;inset:0;border-radius:9999px;background:linear-gradient(180deg,rgba(0,0,0,0.20),rgba(0,0,0,0.10));width:calc(100% - var(--shadow-cutoff-fix) - 0.25em);height:calc(100% - var(--shadow-cutoff-fix) - 0.25em);top:calc(var(--shadow-cutoff-fix) - 0.5em);left:calc(var(--shadow-cutoff-fix) - 0.875em);padding:0.125em;box-sizing:border-box;mask:linear-gradient(#000 0 0) content-box,linear-gradient(#000 0 0);mask-composite:exclude;transition:all var(--anim-time) var(--anim-ease);opacity:1}
        .glass-button{-webkit-tap-highlight-color:transparent;backdrop-filter:blur(clamp(1px,0.125em,4px));transition:all var(--anim-time) var(--anim-ease);background:linear-gradient(-75deg,rgba(255,255,255,0.05),rgba(255,255,255,0.22),rgba(255,255,255,0.05));box-shadow:inset 0 0.125em 0.125em rgba(0,0,0,0.05),inset 0 -0.125em 0.125em rgba(255,255,255,0.50),0 0.25em 0.125em -0.125em rgba(0,0,0,0.20),0 0 0.1em 0.25em inset rgba(255,255,255,0.22),0 0 0 0 white}.glass-button:hover{transform:translateY(1px);backdrop-filter:blur(clamp(1px,0.08em,3px));box-shadow:inset 0 0.25em 0.35em 0 rgba(0,0,0,0.18),inset 0 0.05em 0.1em 0 rgba(0,0,0,0.10),inset 0 -0.05em 0.05em 0 rgba(255,255,255,0.18),0 0 0 0 white}.glass-button-text{color:rgba(0,0,0,0.88);text-shadow:0 0.25em 0.05em rgba(0,0,0,0.10);transition:all var(--anim-time) var(--anim-ease)}.glass-button:hover .glass-button-text{text-shadow:0.025em 0.025em 0.025em rgba(0,0,0,0.12)}.glass-button-text::after{content:"";display:block;position:absolute;width:calc(100% - var(--border-width));height:calc(100% - var(--border-width));top:calc(0% + var(--border-width)/2);left:calc(0% + var(--border-width)/2);box-sizing:border-box;border-radius:9999px;overflow:clip;background:linear-gradient(var(--angle-2),transparent 0%,rgba(255,255,255,0.50) 40% 50%,transparent 55%);z-index:3;mix-blend-mode:screen;pointer-events:none;background-size:200% 200%;background-position:0% 50%;transition:background-position calc(var(--anim-time)*1.25) var(--anim-ease),--angle-2 calc(var(--anim-time)*1.25) var(--anim-ease)}.glass-button:hover .glass-button-text::after{background-position:25% 50%}.glass-button:active .glass-button-text::after{background-position:50% 15%;--angle-2:-15deg}.glass-button::after{content:"";position:absolute;z-index:1;inset:0;border-radius:9999px;width:calc(100% + var(--border-width));height:calc(100% + var(--border-width));top:calc(0% - var(--border-width)/2);left:calc(0% - var(--border-width)/2);padding:var(--border-width);box-sizing:border-box;background:conic-gradient(from var(--angle-1) at 50% 50%,rgba(0,0,0,0.50) 0%,transparent 5% 40%,rgba(0,0,0,0.50) 50%,transparent 60% 95%,rgba(0,0,0,0.50) 100%),linear-gradient(180deg,rgba(255,255,255,0.50),rgba(255,255,255,0.50));mask:linear-gradient(#000 0 0) content-box,linear-gradient(#000 0 0);mask-composite:exclude;transition:all var(--anim-time) var(--anim-ease),--angle-1 500ms ease;box-shadow:inset 0 0 0 calc(var(--border-width)/2) rgba(255,255,255,0.50);pointer-events:none}.glass-button:hover::after{--angle-1:-125deg}.glass-button:active::after{--angle-1:-75deg}.glass-button-wrap:has(.glass-button:hover) .glass-button-shadow{filter:blur(clamp(1px,0.03em,3px))}.glass-button-wrap:has(.glass-button:hover) .glass-button-shadow::after{top:calc(var(--shadow-cutoff-fix) - 0.25em);opacity:0.3}.glass-button-wrap:has(.glass-button:active) .glass-button-shadow{filter:blur(clamp(2px,0.125em,12px))}.glass-button-wrap:has(.glass-button:active) .glass-button-shadow::after{top:calc(var(--shadow-cutoff-fix) - 0.5em);opacity:0.75}.glass-button-wrap:has(.glass-button:active) .glass-button-text{text-shadow:0.025em 0.25em 0.05em rgba(0,0,0,0.12)}.glass-button-wrap:has(.glass-button:active) .glass-button{box-shadow:inset 0 0.125em 0.125em rgba(0,0,0,0.05),inset 0 -0.125em 0.125em rgba(255,255,255,0.50),0 0.125em 0.125em -0.125em rgba(0,0,0,0.20),0 0 0.1em 0.25em inset rgba(255,255,255,0.22),0 0.225em 0.05em 0 rgba(0,0,0,0.05),0 0.25em 0 0 rgba(255,255,255,0.75),inset 0 0.25em 0.05em 0 rgba(0,0,0,0.15)}
        .glass-input-wrap{position:relative;z-index:2;transform-style:preserve-3d;border-radius:9999px}.glass-input{display:flex;position:relative;width:100%;align-items:center;gap:0.5rem;border-radius:9999px;padding:0.25rem;-webkit-tap-highlight-color:transparent;backdrop-filter:blur(clamp(1px,0.125em,4px));transition:all 400ms cubic-bezier(0.25,1,0.5,1);background:linear-gradient(-75deg,rgba(255,255,255,0.05),rgba(255,255,255,0.22),rgba(255,255,255,0.05));box-shadow:inset 0 0.125em 0.125em rgba(0,0,0,0.05),inset 0 -0.125em 0.125em rgba(255,255,255,0.50),0 0.25em 0.125em -0.125em rgba(0,0,0,0.20),0 0 0.1em 0.25em inset rgba(255,255,255,0.22),0 0 0 0 white}.glass-input-wrap:focus-within .glass-input{backdrop-filter:blur(0.01em);box-shadow:inset 0 0.125em 0.125em rgba(0,0,0,0.05),inset 0 -0.125em 0.125em rgba(255,255,255,0.50),0 0.15em 0.05em -0.1em rgba(0,0,0,0.25),0 0 0.05em 0.1em inset rgba(255,255,255,0.50),0 0 0 0 white}.glass-input::after{content:"";position:absolute;z-index:1;inset:0;border-radius:9999px;width:calc(100% + clamp(1px,0.0625em,4px));height:calc(100% + clamp(1px,0.0625em,4px));top:calc(0% - clamp(1px,0.0625em,4px)/2);left:calc(0% - clamp(1px,0.0625em,4px)/2);padding:clamp(1px,0.0625em,4px);box-sizing:border-box;background:conic-gradient(from var(--angle-1) at 50% 50%,rgba(0,0,0,0.50) 0%,transparent 5% 40%,rgba(0,0,0,0.50) 50%,transparent 60% 95%,rgba(0,0,0,0.50) 100%),linear-gradient(180deg,rgba(255,255,255,0.50),rgba(255,255,255,0.50));mask:linear-gradient(#000 0 0) content-box,linear-gradient(#000 0 0);mask-composite:exclude;transition:all 400ms cubic-bezier(0.25,1,0.5,1),--angle-1 500ms ease;box-shadow:inset 0 0 0 calc(clamp(1px,0.0625em,4px)/2) rgba(255,255,255,0.50);pointer-events:none}.glass-input-wrap:focus-within .glass-input::after{--angle-1:-125deg}.glass-input-text-area{position:absolute;inset:0;border-radius:9999px;pointer-events:none}.glass-input-text-area::after{content:"";display:block;position:absolute;width:calc(100% - clamp(1px,0.0625em,4px));height:calc(100% - clamp(1px,0.0625em,4px));top:calc(0% + clamp(1px,0.0625em,4px)/2);left:calc(0% + clamp(1px,0.0625em,4px)/2);box-sizing:border-box;border-radius:9999px;overflow:clip;background:linear-gradient(var(--angle-2),transparent 0%,rgba(255,255,255,0.50) 40% 50%,transparent 55%);z-index:3;mix-blend-mode:screen;pointer-events:none;background-size:200% 200%;background-position:0% 50%;transition:background-position calc(400ms*1.25) cubic-bezier(0.25,1,0.5,1),--angle-2 calc(400ms*1.25) cubic-bezier(0.25,1,0.5,1)}.glass-input-wrap:focus-within .glass-input-text-area::after{background-position:25% 50%}
      `}</style>
              ...
              <motion.div key="password-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
                ...
                <BlurFade delay={0.25}>
                  <p className="text-sm font-medium text-muted-foreground">
                    {mode === 'login' ? 'Enter the password for your account.' : 'Choose a password at least 6 characters long.'}
                  </p>
                </BlurFade>
              </motion.div>
            )}
            {/* Confirm password step (signup only) */}
            {authStep === "confirmPassword" && (
              <motion.div key="confirm-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
                <BlurFade delay={0} className="w-full">
                  <div className="text-center">
                    <p className="font-serif font-light text-4xl sm:text-5xl tracking-tight text-foreground whitespace-nowrap">
                      {headings.confirmPassword}
                    </p>
                  </div>
                </BlurFade>
                <BlurFade delay={0.25}>
                  <p className="text-sm font-medium text-muted-foreground">Confirm your password to create your account.</p>
                </BlurFade>
              </motion.div>
            )}
          </AnimatePresence>
                            ...
                            <div className="glass-input">
                              ...
                              <div className={cn("relative z-10 flex-shrink-0 overflow-hidden transition-all duration-300 ease-in-out", isPasswordValid ? "w-10 pr-1" : "w-0")}>
                                <GlassButton
                                  ...
                                  onClick={mode === 'login' ? undefined : handleProgressStep}
                                  size="icon"
                                  aria-label={mode === 'login' ? "Log in" : "Continue"}
                    ...
                    <div className="glass-input-wrap w-[300px]">
                      <div className="glass-input">
                        ...
                        <input
                          ...
                          onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); if (isConfirmPasswordValid) handleSignupSubmit(); }}}
                          className="relative z-10 h-full w-0 flex-grow bg-transparent text-foreground placeholder:text-foreground/60 focus:outline-none"
                        />
              ...
              <p className="text-sm text-muted-foreground">
                {mode === 'login' ? (
                  <>Don&apos;t have an account?{' '}
                    <Link href="/signup" className="font-semibold text-[#3B82F6] hover:underline">Sign up free</Link>
                  </>
                ) : (
                  <>Already have an account?{' '}
                    <Link href="/login" className="font-semibold text-[#3B82F6] hover:underline">Log in</Link>
                  </>
                )}
              </p>
            </BlurFade>
